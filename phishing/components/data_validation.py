...
class DataValidation:
    ...
    def drop_missing_values_columns(self,df:pd.DataFrame,report_key_name:str)->Optional[pd.DataFrame]:
        """
        This Func. will drip columns which contains missing value more than specific threshold
        ...
        ***************************************************************************************
        ...
        try:
            ...
            ##selcting column names that contains null values
            logging.info(f"selcting column names that contains null values above to {threshold}")
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info(f"Relace na value in base df ")
            #base dataframe has na as null
            logging.info(f"Dropping null Columns from base DataFrame")
            base_df = self.drop_missing_values_columns(df=base_df,report_key_name="missing_values_within_base_dataset")

            logging.info(f"Reading Train Datafeame")
            train_df = pd.read_csv(self.data_ingestion_artifact.train_file_path)
            logging.info(f"Reading Test DataFrame")
            test_df = pd.read_csv(self.data_ingestion_artifact.test_file_path)

            logging.info(f"Dropping Null values from train df")
            ...
            logging.info(f"DataValidation Artifact {data_validation_artifact}")
